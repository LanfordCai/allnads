...
function checkRequiredEnvVars() {
  ...
  if (!process.env.MONAD_TESTNET_RPC) {
    console.warn('Warning: MONAD_TESTNET_RPC environment variable is not set');
  }
}
...
function printUsage() {
  console.log(`
Usage: 
  npm run add-admin-v2 -- [network] [admin_address]

Examples:
  npm run add-admin-v2 -- localhost 0x1234...5678
  npm run add-admin-v2 -- monadTestnet 0xabcd...ef01
...
function getAirdropperAddress(networkName: string): Address {
  try {
    ...
    const airdropperV2Address = deployedAddresses["AllNadsAirdropperV2#AllNadsAirdropper"];
    
    if (!airdropperV2Address) {
      throw new Error(`AllNadsAirdropperV2 contract address not found in deployed_addresses.json for network ${networkName}`);
    }
    
    return airdropperV2Address as Address;
  } catch (error) {
...
async function main() {
  ...
  const airdropperAddress = getAirdropperAddress(networkName);
  console.log(`Using AllNadsAirdropperV2 contract address: ${airdropperAddress}`);
  ...
  try {
    ...
    if (txHash) {
      console.log(`\nSuccess! Added ${adminAddress} as an admin to the AllNadsAirdropperV2 contract`);
